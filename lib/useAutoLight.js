...
export function useAutoLight(count, extraPause = false, { cycle = true } = {}) {
  ...
  useEffect(() => {
    if (!cycle || paused || extraPause || visible.length < 1) {
      if (!paused && !extraPause && cycle) setLit(-1);
      return undefined;
    }
  ...
  }, [cycle, paused, extraPause, visible]);
